...
function StoriesPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      let url = isAuthenticated && userId ? `/api/feed?userId=${userId}` : "/api/stories";
      if (searchQuery) {
...
function StoryCard({ story, isFavorite, onFavoriteClick, onViewStory, onViewProfile, showProgress, formatDate, isAuthenticated }) {
  return (
    <Card
      sx={{
        height: "100%",
    ...
    >
      ...
      <CardContent sx={{ flexGrow: 1, p: 2 }}>
        ...
        {story.author && (
          ...
          >
            <Avatar src={story.author.picture} alt={story.author.name} sx={{ width: 24, height: 24 }} />
            ...
            {story.isFromFollowed && (
              <Chip label="Following" size="small" sx={{ fontSize: "0.6rem", height: 16, bgcolor: "rgba(232, 184, 109, 0.2)", color: "primary.main" }} />
            )}
          </Stack>
        )}
        ...
        <Stack direction="row" spacing={1} flexWrap="wrap" gap={0.5}>
          {story.artStyle && (
            <Chip
              ...
              size="small"
              sx={{ bgcolor: "rgba(232, 184, 109, 0.15)", color: "primary.main", fontSize: "0.7rem" }}
            />
          )}
          <Chip
